...
static void spm_trigger_wfi_for_sleep(struct pwr_ctrl *pwrctrl)
{
	...
	if (spm_dormant_sta < 0) {
		...
		pr_info("[name:spm&][SPM] spm_dormant_sta %d"
			, spm_dormant_sta);
	}
...
unsigned int spm_go_to_sleep(void)
{
	...
	if (!wd_ret) {
		...
		pr_info("[name:spm&]FAILED TO GET WD API\n");
#endif
	...
	pr_info("[name:spm&][SPM] sec = %u, wakesrc = 0x%x (%u)(%u)\n",
		  sec, pwrctrl->wake_src, is_cpu_pdn(pwrctrl->pcm_flags),
		  is_infra_pdn(pwrctrl->pcm_flags));
	...
	if (mtk8250_request_to_sleep()) {
		...
		pr_info("[name:spm&]Fail to request uart sleep\n");
		goto RESTORE_IRQ;
	}
#endif
...
RESTORE_IRQ:
	...
	if (!wd_ret) {
		if (!pwrctrl->wdt_disable)
			...
			aee_sram_printk("pwrctrl->wdt_disable %d\n",
				pwrctrl->wdt_disable);
			pr_info("[name:spm&][SPM] pwrctrl->wdt_disable %d\n",
				pwrctrl->wdt_disable);
		}
	...
	if (pwrctrl->wakelock_timer_val) {
		aee_sram_printk("#@# %s(%d) calling spm_pm_stay_awake()\n",
			__func__, __LINE__);
		pr_info("[name:spm&][SPM ]#@# %s(%d) calling spm_pm_stay_awake()\n",
			__func__, __LINE__);
